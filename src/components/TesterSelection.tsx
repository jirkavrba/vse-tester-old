            ...
            <div className={`absolute top-full right-0 py-3 ${expanded ? 'visible' : 'invisible'}`} onMouseLeave={() => setExpanded(false)}>
                {
                    sets
                        ...
                        .map(subject =>
                            <div key={subject.title} onClick={() => select(subject)} className={`flex flex-col cursor-pointer px-5 py-3 mb-3 bg-white border-2 border-gray-200 rounded-lg transition transform group ${expanded ? 'shadow-lg opacity-1 translate-y-0 hover:bg-white hover:border-black' : 'opacity-0 -translate-y-10'}`}>
