...
const TesterSelection: React.FC<TesterSelectionProps> = ({ sets, selected, onSelect }: TesterSelectionProps) => {
    ...
    return (
        <div className="relative flex flex-col top-0 right-0 transition items-end justify-start" onMouseLeave={() => setExpanded(false)}>
            ...
            <div className={`absolute top-full right-0 py-3 ${expanded ? 'visible' : 'invisible'}`} onMouseLeave={() => setExpanded(false)}>
                {
                    sets
                        ...
                        .map(subject =>
                            <div key={subject.title} onClick={() => select(subject)} className={`flex flex-col cursor-pointer px-5 py-3 mb-3 bg-neutral-800 border-2 border-neutral-700 rounded-lg transition transform group ${expanded ? 'shadow-lg opacity-1 translate-y-0 hover:bg-neutral-900 hover:border-white' : 'opacity-0 -translate-y-10'}`}>
                                <h1 className="transition text-neutral-300 group-hover:text-white uppercase font-black tracking-widest">{subject.subject}</h1>
